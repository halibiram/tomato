...
print_info "Installing required packages: python3, python3-venv, nodejs, npm, nginx, wireguard..."
apt-get install -y python3 python3-venv nodejs npm nginx wireguard
...
# 3. Setup Backend in a Virtual Environment
print_info "Setting up Python backend in a virtual environment..."
cd $PROJECT_DIR
python3 -m venv venv
...
# Generate password hash using the venv's python
ADMIN_PASSWORD_HASH=$($PROJECT_DIR/venv/bin/python3 -c "from backend.security import get_password_hash; print(get_password_hash('$ADMIN_PASSWORD'))")
...
# Create the initial database using the venv's python
print_info "Creating database..."
$PROJECT_DIR/venv/bin/python3 -c "from backend.database import Base, engine; Base.metadata.create_all(bind=engine)"
...
ExecStart=$PROJECT_DIR/venv/bin/python3 -m uvicorn main:app --host 0.0.0.0 --port 8000
Restart=always
...
(crontab -l 2>/dev/null; echo "0 3 * * * $PROJECT_DIR/venv/bin/python3 $BACKEND_DIR/youtube_engine/cdn_discovery.py >> /var/log/yt_ip_discovery.log 2>&1") | crontab -
